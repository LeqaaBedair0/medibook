...
function ExceptionManager({ isOpen, onClose, exceptions, onUpdate, doctorId, allClinics }) {
  ...
  const handleAddException = async () => {
    try {
      ...
      const response = await fetch(`http://13.63.47.45:8000/api/doctor/${doctorId}/exceptions`, {
        method: 'POST',
  ...
  const handleDeleteException = async (exceptionId) => {
    ...
    try {
      const response = await fetch(`http://13.63.47.45:8000/api/doctor/${doctorId}/exceptions/${exceptionId}`, {
        method: 'DELETE'
      });
